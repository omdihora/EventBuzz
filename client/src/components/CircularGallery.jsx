...
class GalleryApp {
    constructor(container, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, autoScrollSpeed = 0.4 } = {}) {
    ...
    update() {
        // Auto-scroll: continuously move right-to-left unless hovering or dragging
        if (!this.isHovering && !this.isDown) {
            this.scroll.target += this.autoScrollSpeed * 0.016; // ~per frame at 60fps
        }
...
export default function CircularGallery({
    ...
    useEffect(() => {
        const app = new GalleryApp(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, autoScrollSpeed });
        return () => { app.destroy(); };
